...
export function ClientWIPCases() {
  ...
  return (
    <div className="p-6">
      <TopBar title="WIP Cases" breadcrumb={['Client', 'WIP Cases']} />
      ...
      <div className="mt-6 bg-white rounded-xl border border-slate-200">
        <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between gap-4">
          ...
          <span className="text-xs text-slate-400 font-jakarta bg-slate-100 px-2 py-1 rounded-full">{wipCases.length} active</span>
        </div>
        ...
        {wipCases.length === 0 ? (
          <EmptyState
            ...
            description="Cases will appear here once employees submit their documents."
          />
        ) : (
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-slate-50">
                  {['#', 'Employee', 'Code', 'BGV Checks', 'Submitted', 'Documents'].map(h => (
                  ...
                  <tr key={c.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-6 py-3">
                      <div className="flex flex-wrap gap-1">
                        {c.bgvChecks.slice(0, 3).map(check => (
                          <span key={check} className="px-2 py-0.5 rounded-full bg-[#2563EB]/10 text-[#2563EB] text-xs font-jakarta">{check}</span>
                        ))}
                        {c.bgvChecks.length > 3 && (
                          <span className="px-2 py-0.5 rounded-full bg-slate-100 text-slate-500 text-xs font-jakarta">+{c.bgvChecks.length - 3}</span>
                        )}
                      </div>
                    </td>
